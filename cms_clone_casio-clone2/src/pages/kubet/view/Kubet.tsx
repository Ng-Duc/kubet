...
import { getListUserKubet, removeUser } from '../api';
import moment from 'moment';
import { APP_CONFIG } from 'utils/env';
import { deleteAllUser } from 'pages/thabet/api';
...
const Kubet = () => {
  ...
  const handleGetData = async () => {
    const res = await getListUserKubet({ casio: 'kubet' });
  ...
  const handleDeleteAll = async () => {
    try {
      const res = await deleteAllUser('kubet');
      notification.success({ message: 'Xoá thành công' });
      handleGetData();
    } catch (error) {
      notification.error({ message: 'Lỗi xoá' });
  ...
  return (
    <Space className="dashboard full-width" direction="vertical" size={15}>
      <div className="dashboard__wrap">
        <Typography.Title level={3}>List User Kubet</Typography.Title>
