...
import { deleteUserRegister, dowloadFile, getListUserThabetRegister, removeUserRegister } from '../api';
...
const ThabetRegister = () => {
  ...
  const handleGetData = async () => {
    try {
      const res = await getListUserThabetRegister({ casio: 'thabet' });
  ...
  const handleDeleteAll = async () => {
    try {
      const res = await deleteUserRegister('thabet');
      notification.success({ message: 'Xoá thành công' });
      handleGetData();
    } catch (error) {
      notification.error({ message: 'Lỗi xoá' });
  ...
  return (
    <Space className="dashboard full-width" direction="vertical" size={15}>
      <div className="dashboard__wrap">
        <Typography.Title level={3}>List User Register Thabet</Typography.Title>
