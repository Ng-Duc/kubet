...
import { dowloadFile, getListUserThabetRegister, removeUserRegister } from '../api';
import moment from 'moment';
import { APP_CONFIG } from 'utils/env';
import { deleteUserRegister } from 'pages/thabetRegister/api';
...
const KubetRegister = () => {
  ...
  const handleGetData = async () => {
    try {
      const res = await getListUserThabetRegister({ casio: 'kubet' });
  ...
  const handleRemoveUser = async (id: string) => {
    try {
      const res = await removeUserRegister(id);
  ...
  const handleExportFile = async () => {
    const casio = 'kubet';
    try {
      window.open(APP_CONFIG.apiUrl + `/export-to-excel-register?casio=${casio}`, '_blank');
  ...
  const handleDeleteAll = async () => {
    try {
      const res = await deleteUserRegister('kubet');
      notification.success({ message: 'Xoá thành công' });
      handleGetData();
    } catch (error) {
      notification.error({ message: 'Lỗi xoá' });
  ...
  return (
    <Space className="dashboard full-width" direction="vertical" size={15}>
      <div className="dashboard__wrap">
        <Typography.Title level={3}>List User Register Kubet</Typography.Title>
